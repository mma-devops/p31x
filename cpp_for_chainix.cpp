// ДЛЯ VISUAL STUDIO руссиш лангуаге:
...
int main()
{
    ...
    // ссылки и указатели
    // ссылки:
    // создать ссылку: int &название_ссылки = другая перменная
    // просмотреть адрес в памяти: cout << &a;
    // допустим у вас есть переменная и вам нужно продублировать её значение

    int a = 100; // int - 4 байта
    int b = a;   // int - 4 байта
    // сумма 8, бесмысленное затрачивание памяти на хранение двух одинаковых значений

    // при выводе адресов:
    cout << &a << endl
         << &b;
    // 1c23f004 - адрес значения a
    // 1c23f000 - адрес значения b
    // можно заметить что разница адресов 4 байта, как и тип int

    // вместо того чтобы создавать новую переменную и присваивать ей другую, можно использовать ссылку:
    int a = 100;
    int &b = a;
    // при выводе адресов:
    cout << &a << endl << &b;
    // 1c23f004 - адрес значения 'a'
    // 1c23f004 - адрес значения 'b'
    // адреса одинаковые, так как 'b' является ссылкой на адрес 'a'

    // так же если прибавить к 'a', допустим, единицу, то к 'b' тоже прибавится единица
    // так как адрес не изменился, а изменилось только значение в этои адресе
    int a = 100;
    int &b = a;
    a++;
    cout << a << endl
         << b;
    // 101 - значение 'a'
    // 101 - так как 'b' - ссылка на 'a', то выведется то же значение что и в 'a'

    // указатели:
    // создать указатель: int* название_указателя = &другая перменная (АДРЕС ПЕРЕМЕННОЙ)
    // по сути то же самое, только по укзателю мы можем менять значение переменной:
    int a = 100;
    int* b = &a;
    a++;
    *b+=1; // *b++ НЕРАБОТАЕТ так как вот так вот
    cout << a << endl << *b;

    // 102
    // 102 в обоих случаях, так как мы прибавили единицу 2 раза в одну и ту же ячейку памяти

    // в первую очередь это используется для оптимизации кода
    // допустим я написал функцию для нахождения минимального элемента
    // допустим массив состоит из миллиарда элементов, НО когда я передаю этот массив в функцию,
    // функция создаёт КОПИЮ этого массива, а это дохрена памяти будет задействовать
    // вместо этого я могу передавать в функцию не массив, а указатель на первый элемент

    void min(int *arr, int N)
    {                   //  передаю указатель на первый элемент массива и размер массива
        int min = *arr; // допустим первый элемент массива имеет адрес 1c23f000
        for (int i = 0; i < N; i++)
        {
            if (*(arr + i) < min)
            { // следующий элемент *(arr+i) будет иметь адрес 1c23f004 (на 4 байта больше, так как значения типа int)
                min = *(arr + i)
            }
        }
        cout << min;
...
// Классы:
// ет такая структура которая "действует как тип переменной" и хранит в себе какие-то свойства которые мы может прописать
// допустим создадим класс для какого-нибудь NPC:
class NPC{ // создали клас
public: // задаём область видимости (далее разберем другие, пока что используем public)
    string name; // нпс имеет имя
    int lvl, hp, dmg; // уровень, здоровье, урон
    bool interactable; // можно ли с ним взаимодействовать
    // так же внутри классов можно писать МЕТОДЫ (функции) этих классов
    void printInfo(){
        printf("\nName: %s\nLevel: %d\nHealth: %d\nDamage: %d\nIncteractable: %s\n", name, lvl, hp, dmg, interactable? "true":"false");
    }
}; // ВНИМАНИЕ!!!!!!!!!!!! после фигурных скобок класса ставится ТОЧКА С ЗАПЯТОЙ (;);;;;;;;;;;;;;;;;;;;;;;
int main(){
    NPC actor; // создаём объект с названием "actor" класса "NPC"
    actor.name = "Billy Herrington"; // даём нашему объектту свойство имени
    actor.lvl = 48; // устанавливаем уровень объекта
    actor.hp = 1337; // хп
    actor.dmg = 2018; // урон
    actor.interactable = true; // взаимодействие с персонажем (oh my)
    //можем вызвать метод класса для вывода инфы конкретного объекта:
    // объект.метод();
    actor.printInfo();
}